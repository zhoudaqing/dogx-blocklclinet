...
Utils.privateKeyToAESKey = function(privKey) {
  ...
  $.checkArgument(Digibyte.PrivateKey.isValid(privKey), 'The private key received is invalid');
  var pk = Digibyte.PrivateKey.fromString(privKey);
  return Digibyte.crypto.Hash.sha256(pk.toBuffer()).slice(0, 16).toString('base64');
};
