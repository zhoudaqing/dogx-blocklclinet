...
PayPro.get = function(opts, cb) {
  ...
  opts.headers = opts.headers || {
    'Accept': BitcorePayPro.LEGACY_PAYMENT.REQUEST_CONTENT_TYPE,
  ...
  http(opts, function(err, dataBuffer) {
    ...
    var addr = new Digibyte.Address.fromScript(new Digibyte.Script(scriptBuf), network);
...
PayPro._getPayProRefundOutputs = function(addrStr, amount) {
  ...
  if (addr.isPayToPublicKeyHash()) {
    s = Digibyte.Script.buildPublicKeyHashOut(addr);
  } else if (addr.isPayToScriptHash()) {
    s = Digibyte.Script.buildScriptHashOut(addr);
  } else {
...
PayPro.send = function(opts, cb) {
  ...
  opts.method = 'POST';
  opts.headers = opts.headers || {
    'Accept': BitcorePayPro.LEGACY_PAYMENT.ACK_CONTENT_TYPE,
    'Content-Type': BitcorePayPro.LEGACY_PAYMENT.CONTENT_TYPE,
    // 'Content-Type': 'application/octet-stream',
  };
  opts.body = payment;
